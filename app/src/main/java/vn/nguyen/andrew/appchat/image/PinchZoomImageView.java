...
/**
 * Created by trunganh on 12/10/2016.
 */
public class PinchZoomImageView extends ImageView {
    ...
    public void setImageBitmap(Bitmap bm) {
        try {
            float aspecRatio = (float) bm.getHeight() / (float) bm.getWidth();
            DisplayMetrics displayMetrics = getResources().getDisplayMetrics();
            mImageWidth = displayMetrics.widthPixels;
            mImageHeight = Math.round(mImageWidth * aspecRatio);
            mBitmap = Bitmap.createScaledBitmap(bm, mImageWidth, mImageHeight, false);
            invalidate();
            requestLayout();
        }catch (Exception e){
            e.printStackTrace();
